...
// 移植自桌面端 src/timeParser.js，规则保持一致。
...
function parseWhen(raw, base = new Date()) {
  let text = String(raw || "")
  ...
  let rel = null;
  for (const pair of relMap) { const m = text.match(pair[0]); if (m) { rel = { m, off: pair[1] }; break; } }
  ...
  if (rel) {
    ...
    const d = new Date(base.getFullYear(), base.getMonth(), +dayOnly[2]);
    date = d2s(d.getFullYear(), d.getMonth() + 1, d.getDate());
    if (d < new Date(base.getFullYear(), base.getMonth(), base.getDate())) {
      const n = new Date(base.getFullYear(), base.getMonth() + 1, +dayOnly[2]);
      date = d2s(n.getFullYear(), n.getMonth() + 1, n.getDate());
    }
    // 前面的捕获组只是「分隔符断言」，别把它从标题里吃掉
    eaten.push(dayOnly[0].slice(dayOnly[1].length));
  }
...
function dateOf(offsetDays, base = new Date()) {
...
function guessQuad(dateStr, base = new Date()) {
  if (!dateStr) return 2;
...
module.exports = { parseWhen, dateOf, guessCategory, guessQuad };
