// Generated by tools/sync-care-model.js; edit le-time-management/src/careModel.js.
...
function addReminder(c, patch, now = new Date()) {
...
function recordAction(c, key, action, now = Date.now()) {
...
function dueItems(c, now = new Date()) {
  return todayItems(c, now).filter(r => r.status !== "done" && r.dueAt <= now.getTime() && !r.deliveredAt);
}
function reminderPayload(r, deviceId, now = Date.now()) {
...
module.exports = { dayOf, careState, addReminder, occurs, todayItems, recordAction, dueItems, reminderPayload };
