...
> 适用：Le时间管理 0.1.0+ ｜ 对应源码：`src/icons.js`、`src/shell.js`、`src/views/settings.js`、`public/icons/`
...
| 位置 | `le-time-management/public/icons/` | Vite 会把 `public/` 原样拷到产物根，运行时路径 `/icons/<key>.png` |
...
**2. 生成 128×128 RGBA PNG**（脚本会把 `cdn-icons-png.magnific.com/512/<id//1000>/<id>.png` 缩放后写入 `le-time-management/public/icons/<key>.png`）：
...
src.resize((128, 128), Image.Resampling.LANCZOS).save("le-time-management/public/icons/cn-holiday.png")
```
...
```powershell
# 尺寸与位深
$b=[IO.File]::ReadAllBytes("le-time-management/public/icons/cn-holiday.png")
...
| Web 页签图标 | `index.html` 里 `<link rel="icon" href="/app-icon.png" />` | **当前 `le-time-management/public/app-icon.png` 不存在**，浏览器会 404 → 放一张 PNG（建议 512×512）到 `public/app-icon.png`，或把这一行改指向已存在的图标 |
...
```powershell
(Get-FileHash le-time-management/public/icons/cn-holiday.png -Algorithm SHA256).Hash.ToLower()
```
...
- [ ] 文件名 = 插件 `id`，全小写，放 `le-time-management/public/icons/`
